...
class MyStack extends TerraformStack {
  constructor(scope: Construct, id: string) {
    ...
    const awsProvider = new AwsProvider(this, "AWS", {
      region: "us-east-1",
      accessKey: "ASIAWPRT43PNFNFTUZ7C",
      secretKey: "qxDUuT4HSEhq6XRkpbb7UJVj2dlXB1xNrEFmOZY6",
      token: ""
    });
    ...
    const imageId = amiMappings[region];
    ...
    const internalHost = [
      'b-#.',
      Fn.element(Fn.split('*.', mskWildcardDNS.stringValue), 1),
    ].join('');

    // CloudFormation Init configurations
    const zillaYamlContent = Fn.rawString(`
name: public
vaults:
  secure:
    type: aws
bindings:
  ...
  tcp_client:
    ...
    routes:
    - when:
      - authority: "${mskWildcardDNS.stringValue}"
    `);
...
action=/opt/aws/bin/cfn-init -v --stack ${id} --resource MSKProxyLaunchTemplate --region ${region}
runas=root
    `;
    
        // User data script with cfn-init and cfn-signal
...
/opt/aws/bin/cfn-init -v --stack ${id} --resource MSKProxyLaunchTemplate --region ${region}
/opt/aws/bin/cfn-signal -e $? --stack ${id} --resource MSKProxyGroup --region ${region}
    `;
    ...
    const MSKProxyLaunchTemplate = new launchTemplate.LaunchTemplate(this, 'MSKProxyLaunchTemplate', {
      ...
      tags: {
        Name: 'MSKProxyLaunchTemplateTag',
      }
    });

    // Define Auto Scaling Group
    new autoscalingGroup.AutoscalingGroup(this, 'MSKProxyGroup', {
